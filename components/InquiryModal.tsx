...
export default function InquiryModal({
    ...
    const handleEvent = (e: Event) => {
      ...
      if (plan) {
        const match = ALL_PACKAGES.find((p) =>
          p.toLowerCase().includes(plan.toLowerCase())
        );
        setPackageChoice(match || plan);
      }
      setInternalOpen(true);
    };

    window.addEventListener("open-inquiry-modal", handleEvent);
    return () => {
      window.removeEventListener("open-inquiry-modal", handleEvent);
    };
  }, []);
  ...
  useEffect(() => {
    if (propSelectedPlan) {
      const match = ALL_PACKAGES.find((p) =>
        p.toLowerCase().includes(propSelectedPlan.toLowerCase())
      );
      setPackageChoice(match || propSelectedPlan);
    }
  }, [propSelectedPlan]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      ...
      <div
        ...
        className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border border-slate-200 relative z-10 animate-in zoom-in-95 duration-200"
      >
        {/* Modal Header */}
        <div className="bg-[#03183d] text-white p-5 relative border-b-4 border-[#ff5500]">
          ...
          <h3 id="modal-title" className="text-xl font-black uppercase tracking-tight text-white">
            GET NEW CONNECTION
          </h3>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          {/* Full Name */}
          <div>
            <label htmlFor="modal-name" className="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2.5 text-xs sm:text-sm font-semibold rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#0047bb]/20 focus:border-[#0047bb] text-slate-800 placeholder-slate-500"
              />
            </div>
          </div>
          ...
          {/* Mobile Number */}
          <div>
            <label htmlFor="modal-mobile" className="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1">
          ...
          {/* Email Address */}
          <div>
            <label htmlFor="modal-email" className="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1">
              Email Address
            </label>
            <div className="relative">
          ...
          {/* Address */}
          <div>
            <label htmlFor="modal-address" className="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1">
          ...
          {/* Package Selection */}
          <div>
            <label htmlFor="modal-package-select" className="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1">
              Select Package / Plan
            </label>
            <div className="relative">
              ...
              <select
                ...
                aria-label="Select Package / Plan"
                value={packageChoice}
